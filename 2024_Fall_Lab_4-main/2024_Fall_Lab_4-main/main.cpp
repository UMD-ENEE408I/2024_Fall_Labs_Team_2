...
void loop() {
  ...
  while(true) {
    ...
    for (int i = 0; i < 13; i++) {
      if (lineArray[i] == 1) {
        isCorner = false; // Not a corner if any sensor detects the line
        break;
      }
    }
    
    if (isCorner) {
      // If a corner is detected, stop and turn
      turnCorner(true); // Example: turn clockwise
    }
  }
}
